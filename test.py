...
def run_llm_tools_nmap(target: str, ports: Optional[str], flags: Optional[str]):
    ...
    last_exc = None
    for name, _params in candidates:
        if hasattr(_llm_tools, name):
            ...
            try:
                ...
                sig = None
            try:
                # Build kwargs only for parameters the function accepts
                kwargs = {}
                if sig:
                    for p in sig.parameters.values():
                        if p.name == "target":
                            ...
                            kwargs["flags"] = flags
                # If signature unknown, try keyword call (best-effort)
                if kwargs:
                    return fn(**kwargs)
                else:
                    # last resort: try fewer-positional args patterns
                    try:
